"""Build the final Chinese report and figures directly from verified results."""
import json
...
from research_v2 import ROOT, RESULTS, VALID_END, END
...
def main():
    ...
    audit = json.loads((RESULTS / "v3_audit.json").read_text(encoding="utf-8"))
    boot = pd.read_csv(RESULTS / "v3_bootstrap_ci.csv")
    baseline, cost_rows, intervals = [], [], []
    fig, axes = plt.subplots(2, 2, figsize=(14, 9), constrained_layout=True)
    for rank, cfg in enumerate(chosen[:5], 1):
        rows = costs[costs.selection_rank == rank]
        val = rows[(rows.period == "valid") & (rows.cost_bp == 2)].iloc[0]
        later = rows[(rows.period == "holdout") & (rows.cost_bp == 2)].iloc[0]
        baseline.append([cfg["model"], f"{val['return']:.2%}", f"{later['return']:.2%}",
                         int(later.trades), f"{later.break_even_bp:.2f}"])
        sensitivity = rows[rows.period == "holdout"].sort_values("cost_bp")
        cost_rows.append([cfg["model"]] + [f"{r:.2%}" for r in sensitivity['return']])
        ci = boot[boot['rank'] == rank].iloc[0]
        intervals.append([cfg["model"], f"{ci.bootstrap_ci_2_5:.2%}", f"{ci.bootstrap_ci_97_5:.2%}"])
        stem = f"v3_selected_{rank}_{cfg['model']}_{int(cfg['frequency_min'])}m_2bp"
        eq = load_equity(RESULTS / f"{stem}_equity.csv")
        base, _, _ = period_stats(eq, VALID_END, END)
        daily = eq[(eq.index >= VALID_END) & (eq.index < END)].resample("1D").last()
        axes[0, 0].plot(daily.index, 100 * (daily / base - 1), label=cfg['model'])
        axes[0, 1].plot(sensitivity.cost_bp, sensitivity['return'] * 100, marker="o", label=cfg['model'])
    axes[0, 0].set(title="Fixed controls: July-August equity (2 bp)", ylabel="Return (%)")
    axes[0, 0].legend(fontsize=8)
    axes[0, 1].set(title="One-way all-in cost sensitivity", xlabel="Cost per leg (bp)", ylabel="Return (%)")
    for model, g in grid.groupby("model"):
        axes[1, 0].scatter(g.valid_return * 100, g.holdout_return * 100, label=model, s=20, alpha=.8)
    axes[1, 0].set(title="All 67 configurations: reused evaluation", xlabel="May-June return (%)", ylabel="July-August return (%)")
    axes[1, 0].legend(fontsize=7, ncol=2)
    matched = grid[(grid.model.isin(["B0", "PEER", "MEDIAN", "PCA3"])) &
                   (grid.frequency_min.isin([15, 60])) & (grid.center_hours == 4) &
                   (grid.scale_days == 7) & (grid.center_kind == "sma") &
                   (grid.logic.isin(["band", "wide_exit", "confirm2", "turn", "zero_cross"]))]
    assert len(matched) == 40
    logic = matched.groupby("logic").agg(configs=("model", "size"),
        median_valid=("valid_return", "median"), median_later=("holdout_return", "median"))
    logic.to_csv(RESULTS / "v3_matched_logic_2bp.csv")
    axes[1, 1].bar(logic.index, logic.median_later * 100, color="#5279a5")
    axes[1, 1].set(title="Matched rules: median July-August return", ylabel="Return (%)")
    axes[1, 1].tick_params(axis="x", rotation=20)
    for ax in axes.ravel():
        ax.axhline(0, color="gray", linewidth=.7)
        ax.grid(alpha=.15)
    axes[1, 0].axvline(0, color="gray", linewidth=.7)
    fig.savefig(RESULTS / "v3_overview.png", dpi=160)
    plt.close(fig)
    leader_rows = []
    for cfg in chosen[5:]:
        leader_rows.append([cfg['model'], cfg['frequency_min'], cfg['center_hours'],
                            cfg['center_kind'], cfg['logic'],
                            f"{cfg['valid_return']:.2%}", f"{cfg['holdout_return']:.2%}"])
    logic_rows = [[name, int(r.configs), f"{r.median_valid:.2%}", f"{r.median_later:.2%}"] for name, r in logic.iterrows()]
    max_error = max(abs(x['reconciliation_error']) for x in audit['accounts'])
    text = f'''# v3 多定价公式、周期与开平仓逻辑研究

## 主要发现

完成 67 个唯一配置、8 类信号模型的扩展回测，以及 10 个代表配置的 0—3 bp 成本敏感性。2 bp 下，7—8 月有 {(grid.holdout_return > 0).sum()} 个配置为正；同时在 5—6 月与 7—8 月为正的只有 B0 60 分钟反转确认规则（+0.72%、+1.15%），它是单标的基线。多标的模型在这两个阶段没有同时为正的配置。验证期排名前五的配置在后一阶段全部亏损，当前矩阵没有给出稳定的多标的均值回归优势。
...
{table(['模型', '5—6 月', '7—8 月', '后期交易数', '后期价格盈亏平衡 bp'], baseline)}

以下为这些固定控制在 7—8 月的成本曲线。0—3 bp 是每一条成交腿的单边综合成本假设；没有另外模拟盘口冲击、排队或 maker 返佣。资金费单独进入现金账本。

{table(['模型', '0 bp', '1 bp', '2 bp', '3 bp'], cost_rows)}
...
{table(['固定对照', '2.5%分位', '97.5%分位'], intervals)}
...
24项测试通过。10个账户逐笔价格损益−费用+资金费与最终权益差异最大为 {max_error:.3g}；逐阶段手续费、日收益复合、逐月复合和阶段起止权益也已核对。保存全部67配置日权益，避免为改变统计报表重复交易模拟。源代码和主要结果 SHA256 见 results/v3_audit.json。
...
'''
    ...
    print("Wrote REPORT_V3.md, v3_overview.png and matched-rule summary")
